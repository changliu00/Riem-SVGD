...
int main(int argn, char *argv[])
{
	...
	if(command == "tr") {
		...
		for(const auto bnin: bnins)
			for(const auto N: Ns) {
				tsModel.bnin = bnin; tsModel.N = N; tsModel.EvalPerpSeq();
			}
	} else if(command == "tw") {
		if( (argn < 4) || !pm.Load(argv[3]) || !pm.Adjust(argn, argv, 4) ) {
			PrintUsage(); return 1;
...
void PrintUsage(void)
{
	cout << endl;
	cout << "Usage:" << endl;
	cout << "samsggmc tr [settingsFilename] ([var]) ([val]) (...)" << endl;
	cout << "samsggmc ts [modelDirname] [settingsFilename] ([var]) ([val]) (...)" << endl;
	cout << "samsggmc tw [modelDirname] [settingsFilename] ([var]) ([val]) (...)" << endl;
	cout << "samsggmc re [tsDirname]" << endl;
	cout << endl;
}
